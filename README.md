...
| クエリ | ベクトル | BM25 | SoftMatcha | RRFスコア |
|--------|:--------:|:----:|:----------:|-----------|
| `openclaw gateway restart` | ✅ | ✅ | - | 0.0323（2方式合算） |
| `git rebase インタラクティブ` | ✅ | - | ✅ | 0.0164（SoftMatchaがフレーズ検出） |
| `記憶の自動整理エンジン` | ✅ | ✅ | ✅ | **0.0492（3方式全ヒット、理論最高値）** |

3方式すべてにヒットすると、スコアが単独ヒット時の約3倍になります。各検索方式が異なる角度から結果を補完し合うことで、関連性の高いドキュメントが確実に上位に浮上します。
