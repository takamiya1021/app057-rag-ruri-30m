...
2. ruri+BM25 更新チェック（check_updates を呼ぶ）
   ※ インデックスが既にある場合のみ実行
   - 更新・削除されたファイルがあれば → ユーザーに報告
     「更新されたファイルがあります。インデックスを更新しますか？」
     - はい → sync_updates で差分更新
     - いいえ → スキップ
   - 更新なし → スキップ

3. SoftMatcha 2 更新チェック（rag://softmatcha-status を確認）
   ※ インデックスが既にある場合のみ実行
   - hasIndex === false or 前回構築から24時間以上経過 →
     「SoftMatcha 2のインデックスを更新しますか？」
     - はい → build_softmatcha_index
     - いいえ → スキップ（SoftMatcha抜きでruri+BM25のみで検索）
   - 最新 → スキップ

4. クエリを最適化（上記ルール）
5. MCP kuro-rag-ruri-30m search を呼ぶ
6. 結果をユーザーに分かりやすく回答
7. ヒットしなければキーワードを変えて再検索
```
...
2. **CLI `rag-cli.ts add-dir` を使う**（MCPを通さず直接DB書き込み）
   ...
   - プロジェクトディレクトリ `/home/ustar-wsl-2-2/projects/100apps/app057-rag-ruri-30m` で実行すること
