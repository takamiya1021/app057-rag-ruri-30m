...
| 構造検索 | SoftMatcha 2（セマンティックパターンマッチ） | 文の構造・パターンが似ている表現の検出 |
...
| `search` | トリプルハイブリッド検索（ベクトル + BM25 + SoftMatcha構造検索） |
...
| `rag://softmatcha-status` | SoftMatcha構造検索の状態 |
...
| パッケージ | 用途 |
|-----------|------|
| `softmatcha` | 構造検索エンジン本体（Python + Rust） |
