...
ファイルをインデックスに登録する処理は以下の順序で行われる。

1. **ファイル読み込み** — 対応形式（.txt, .md, .pdf, .json, .csv）のファイルからテキストを抽出
2. **チャンク分割** — テキストを段落ベースで分割（Markdownの場合は見出し単位を考慮）
...
本仕様書ではローカルドライブ上のファイルを対象とする。Google Driveの差分同期については `doc/gdrive-index-guide.md` を参照。
